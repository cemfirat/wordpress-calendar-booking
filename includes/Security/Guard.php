...
class Guard {
    public function checkSubmission(array $post): array {
        ...
        if (!isset($post['cemb_nonce']) || !wp_verify_nonce(sanitize_text_field($post['cemb_nonce']), 'cemb_booking')) {
        ...
        if (!empty($settings['timing_enabled'])) {
            $ts = isset($post['cemb_form_ts']) ? (int) $post['cemb_form_ts'] : 0;
