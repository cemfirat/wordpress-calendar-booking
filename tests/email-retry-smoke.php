...
wpcb_email_retry_assert(
    (int)($GLOBALS['wpcb_email_retry_subject_calls']['Retry DOI'] ?? 0) === 2,
    'Exactly one transport retry occurs for the target DOI after the initial failure.'
);
...
wpcb_email_retry_assert(
    (int)($GLOBALS['wpcb_email_retry_subject_calls']['Retry DOI'] ?? 0) === 2,
    'Repeated queue worker execution does not resend the completed target DOI notification.'
);
