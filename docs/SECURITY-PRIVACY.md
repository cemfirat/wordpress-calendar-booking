...
The HTTP client reads at most one byte beyond the configured body ceiling so an oversized response can be distinguished from a response exactly at the limit. Oversized or excessive responses fail closed and are not parsed as authoritative availability data.
